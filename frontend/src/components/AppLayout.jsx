...
import { createGroup, logoutUser, setPresence } from '../services/api'
...
function preview(content = '') {
  if (content.startsWith('{') && content.includes('"ct"')) return '[Tin nhắn được mã hóa]'
  if (/^[A-Za-z0-9+/=]{20,}\.[A-Za-z0-9+/=]{10,}$/.test(content)) return '[Tin nhắn được mã hóa]'
...
export default function AppLayout() {
  ...
  // Crypto keys — loaded once from localStorage (set at registration)
  const myPrivateJwk = useMemo(() => {
    try { return JSON.parse(localStorage.getItem('crypto_private_jwk') || 'null') } catch { return null }
  }, [])
  const mySelfKey = localStorage.getItem('crypto_self_key') || null
  ...
  const [invisible, setInvisible] = useState(false) // Tàng hình mode

  async function handleTogglePresence() {
    const goingInvisible = !invisible
    setInvisible(goingInvisible)
    try {
      await setPresence(currentUserId, !goingInvisible)
    } catch (err) {
      setInvisible(!goingInvisible)  // revert on failure
      console.warn('[AppLayout] presence toggle failed:', err)
    }
  }
  ...
  function handleLogout() {
    ...
    localStorage.removeItem('crypto_private_jwk')
    localStorage.removeItem('crypto_public_jwk')
    localStorage.removeItem('crypto_self_key')
  ...
  // Map peer_id → parsed public JWK (for encryption)
  const peerPubKeyById = useMemo(() => {
    const map = {}
    for (const p of peers) {
      if (p.public_key) {
        try { map[p.peer_id] = JSON.parse(p.public_key) } catch { /* skip malformed */ }
      }
    }
    return map
  }, [peers])
    ...
    <aside className="flex h-full w-72 shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 transition-colors duration-300">
      <div className="border-b border-slate-200 px-4 py-5 dark:border-slate-800">
        <div className="flex flex-col gap-4">
          ...
          {invisible && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200 text-xs font-semibold text-amber-700 dark:bg-amber-950/20 dark:border-amber-900/40 dark:text-amber-400">
              <span className="inline-flex h-2 w-2 rounded-full bg-amber-500 animate-pulse" />
              Bạn đang tàng hình — peer khác thấy bạn offline
            </div>
          )}
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className={`flex h-7 w-7 items-center justify-center rounded-lg border transition ${
                  invisible
                    ? 'border-amber-300 bg-amber-50 text-amber-600 hover:bg-amber-100 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-400'
                    : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-100'
                }`}
                title={invisible ? 'Bấm để hiện diện trở lại' : 'Bấm để tàng hình (peer khác sẽ thấy bạn offline)'}
              >
                {invisible ? (
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {(() => {
            const totalUnread = Object.values(unreadByPeer).reduce((s, c) => s + c, 0)
            return (
              <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100/70 dark:bg-slate-900/60 border border-slate-200/20">
                <button
                  type="button"
                  onClick={() => setActiveTab('all')}
                  className={`relative flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 ${
                    activeTab === 'all'
                ...
                >
                  Tất cả
                  {totalUnread > 0 && (
                    <span className="inline-flex items-center justify-center min-w-[16px] h-4 px-1 rounded-full bg-red-500 text-white text-[10px] font-black leading-none">
                ...
                >
                  Chưa đọc
                  {totalUnread > 0 && activeTab !== 'unread' && (
        ...
        <div className="flex-1 min-h-0">
          {selectedGroup ? (
            <GroupChat
              ...
              myPrivateJwk={myPrivateJwk}
              mySelfKey={mySelfKey}
              peerPubKeyById={peerPubKeyById}
            />
          ) : activeSelectedPeer ? (
            <ChatWindow
