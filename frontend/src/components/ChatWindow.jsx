...
import { encryptDirect, safeDecryptDirect } from '../services/crypto'
...
export default function ChatWindow({ peer, currentUserId, onToast, messageEvent, deliveryEvent, myPrivateJwk }) {
  ...
  // Parse peer public key once — null if not available (pre-E2EE peer)
  const peerPubJwk = (() => {
    try { return peer.public_key ? JSON.parse(peer.public_key) : null } catch { return null }
  })()

  async function decryptMsg(msg) {
    const plainContent = await safeDecryptDirect(msg.content, myPrivateJwk, peerPubJwk)
    return plainContent === msg.content ? msg : { ...msg, content: plainContent }
  }
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        ...
        // 2. Sync from server (decrypt each message before storing)
        const rawServerMsgs = (await getMessages(peer.peer_id)).filter(m => !m.group_id)
        if (!active) return
        const serverMsgs = await Promise.all(rawServerMsgs.map(m => decryptMsg(m)))
  ...
  useEffect(() => {
    ...
    decryptMsg(messageEvent).then(decrypted => {
  ...
  async function handleSend(e) {
    ...
    // Encrypt before sending over network (falls back to plaintext if no key available)
    const wireContent = (myPrivateJwk && peerPubJwk)
      ? await encryptDirect(content, myPrivateJwk, peerPubJwk)
      : content

    try {
      const result = await sendMessage(currentUserId, peer.peer_id, wireContent)
      const resolved = {
        ...optimistic,
